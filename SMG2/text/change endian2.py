...
def process_hex_data(hex_data: bytes,hex_data3: bytes) -> bytes:
    ...
    pos3 = hex_data3.find(marker)
    if pos3 == -1:
        raise ValueError("找不到标志头 TXT2")

    #print(f"标志头位置：{pos}")

    next_line_pos3 = pos3 + 16  # 下一行开头
    offset_bytes3 = hex_data3[next_line_pos3:next_line_pos3+4]
    offset3 = struct.unpack('>I', offset_bytes3)[0]

    #print(f"偏移量（4字节单位）：{offset}")

    start_pos3 = next_line_pos3 + 4 + offset3 * 4
    if start_pos3 >= len(hex_data3):
        raise ValueError("计算出的处理起始位置超过数据长度")
    ...
    return hex_data3[:start_pos3] + processed_part

def function(source_dir, target_dir, folder3):
    ...
    for file_path in source_dir.rglob('*.msbt'):
        ...
        folder3_p= folder3 / relative_path
        ...
        with open(folder3_p, 'rb') as g:
            data3 = g.read()    
        # 处理
        processed = process_hex_data(data,data3)
...
if __name__ == '__main__':
    ...
    folder2 = '2'
    folder3 = '3'
    function(folder1, folder2, folder3)
